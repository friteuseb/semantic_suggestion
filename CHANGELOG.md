...
line. Verified on running TYPO3 12.4.40 (PHP 8.1), 13.4.22 (PHP 8.2) and 14.3.3 (PHP 8.3)
instances, one of them configured with two sites.

⚠️ **TYPO3 14 users should upgrade**: the backend module could not be opened on that
version.
...
- Upgrade wizard `semanticSuggestionLabelAnalysisRows`, which relabels pre-existing rows. It
  ...
  producers' rows are then indistinguishable; in that case re-run the scheduler task and the
  Solr indexer once and each rewrites its own rows correctly.
...
### Fixed
- **The backend module can be opened on TYPO3 14 again**, by every user including admins.
  The module declared `access => 'user,group'`. TYPO3 14 resolves that value through a
  module access gate registry and returns `false` when nothing matches, so the backend
  redirected to the dashboard with "No module access". Only `user`, `admin` and
  `systemMaintainer` are registered gates, and the value must be a single identifier. It is
  now `'user'`, which is also what the extension effectively had on 12 and 13, where any
  value other than `admin` or `systemMaintainer` falls through to the same generic per-user
  permission check.
...
- **The page tree no longer takes up space in the module on TYPO3 12.**
  `inheritNavigationComponentFromMainModule` was applied only from v13, but the key exists
  since 12.4 — the core uses it in `ext:form` there — and it is the only one that works: a
  submodule of `web` inherits the parent's navigation component unless inheritance is
  switched off, so the `navigationComponent => ''` fallback was ignored. The key is now
  applied unconditionally. The CSS fallback in
  `LegacySemanticBackendController::indexAction()` went with it; it never reached the
  rendered output.
- **Language statistics display real percentages.** Both backend templates computed the
  share as `{count / total * 100}`; Fluid evaluates a single binary operation per
  expression, so the chained form produced an empty value and every row showed 0.0%. The
  percentage now comes from `LanguageService::getLanguageStatistics()`.
...
- Two statements in the module's "How Quality Settings Work" panel now match the code.
  Raising the quality level does not speed up the analysis: the threshold never reaches
  `analyzePages()`, which computes every pair regardless and is filtered only at save time
  (measured on a 56-page tree, 0.2 → 0.7 took the stored rows from 954 to 406 and the
  runtime from 2.11s to 2.06s). And the display threshold is not the only limit on what
  users see — `maxSuggestions` caps the list independently.
